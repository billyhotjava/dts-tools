...
CREATE TABLE ads_budget_overview_kpi (
  ...
  preoccupy_amt             NUMBER(18,2),      -- ods_proj_budget_exec_dtl.adjust_amt（调整金额=预占金额）
...
-- 5.4 ADS-出入库日趋势（近N天由大屏侧过滤或增加 where 条件）
...
SELECT
  ...
  CASE
    ...
    ELSE ROUND(
      SUM(NVL(p.exec_amt, 0) + NVL(p.hist_exec_amt, 0)) / SUM(NVL(p.budget_amt, 0)) * 100,
      2
    )
  END AS exec_rate,
  SUM(NVL(p.budget_amt, 0)) - SUM(NVL(p.exec_amt, 0) + NVL(p.hist_exec_amt, 0)) AS remain_budget,
...
GROUP BY p.proj_code, mp.proj_name;
...
INSERT INTO ads_stock_overview (stat_date, item_cnt, total_onhand, proj_cnt)
SELECT
...
INSERT INTO ads_io_daily_trend (biz_date, in_qty, out_qty)
SELECT
  biz_date,
